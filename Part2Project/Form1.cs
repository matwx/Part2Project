...
namespace Part2Project
{
    public partial class Form1 : Form
    {
        ...
        private void btnAllImages_Click(object sender, EventArgs e)
        {
            ...
            // Load in all the filenames
            if (!Directory.Exists(p + "\\All")) return;

            // Create output directories
            if (!Directory.Exists(p + "\\Sortings")) Directory.CreateDirectory(p + "\\Sortings");
            var sp = p + "\\Sortings";

            string[][] sortedFeatureLists;

            // Create an output directory for Dataset i
            var currInDir = p + "\\All";
            var currOutDir = sp + "\\All";
            if (!Directory.Exists(currOutDir)) Directory.CreateDirectory(currOutDir);
            ...
            for (int featureNum = 0; featureNum < 8; featureNum++)
            {
                ...
                {
                    var pair = jthFeatureSortingList[imageNum];
                    sortedFeatureLists[featureNum][imageNum] = pair.filename.Split('\\').Last().Split('.').First();
                    output += pair.filename.Split('\\').Last().Split('.').First() + nl;
                }
            ...
            for (int i = 0; i < 8; i++)
            {
                for (int j = i + 1; j < 8; j++)
                {
                    ...
                    for (int r = 0; r < 160; r++)
                    {
                        ...
                        // Search for 'searchFor' in the sorted feature list for feature i
                        int jRank = Array.IndexOf(sortedFeatureLists[i], searchFor);
                        int d = jRank - r;
                        sum_d_squared += d * d;
                    }
        ...
        private void btnRoISizeVSBackDist_Click(object sender, EventArgs e)
        {
            dlgFolder.ShowDialog();
            if (dlgFolder.SelectedPath == "") return;
            var p = dlgFolder.SelectedPath;
            ...
            {
                // The iRank is 'r'
                string searchFor = sortedFeatureLists[6][r];
                // Search for 'searchFor' in the sorted feature list for feature i
                int jRank = Array.IndexOf(sortedFeatureLists[4], searchFor);
                toFile += (r+1) + "," + (jRank+1) + Environment.NewLine;
            }

            File.WriteAllText(currOutDir + "\\BackDistRanksForRoISize.txt", toFile);
        }
    }
