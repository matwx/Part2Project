...
namespace Part2Project.Features
{
    static class FeatureBackgroundDistraction
    {
        ...
        {
            DirectBitmap result = new DirectBitmap(4096, 200);
            ...
            using (Graphics gfx = Graphics.FromImage(result.Bitmap))
            {
                for (int i = 0; i < 4096; i++)
                {
                    gfx.DrawLine(Pens.Black, i, 0, i, 200);
                    if (H[i] >= 0.01 * hmax) gfx.DrawLine(Pens.White, i, 200, i, 200 - (int)((double)H[i] / hmax * 200.0));
                }
            }
        ...
        public static DirectBitmap GetQuantisedBackground(DirectBitmap image, bool[][] boundedBinarySaliencyMap)
        {
            ...
            for (int x = 0; x < image.Width; x++)
            {
                for (int y = 0; y < image.Height; y++)
                {
                    if (!boundedBinarySaliencyMap[x][y])
                    {
                        ...
                        result.SetPixel(x, y, Color.FromArgb(r * 15, g * 15, b * 15));
                    }
                    else
                    {
