...
namespace Part2Project.Saliency
{
    class SaliencySegmentation : Segmentation
    {
        ...
        public SaliencySegmentation(Segmentation s, Bitmap image, double sigma)
            : base(s)
        {
            double maxS = 0;
            ...
            using (KalikoImage kImage = new KalikoImage(image))
            {
                kImage.ApplyFilter(new GaussianBlurFilter((float)sigma));
                using (Bitmap blurredImage = new Bitmap(kImage.GetAsBitmap()))
                {
        ...
        public void RenormaliseToIgnore()
        {
        ...
        public Bitmap GetHighlightedSegmentSaliencyMap()
        {
            Bitmap image = new Bitmap(Width, Height);
            ...
            for (int x = 0; x < Width; x++)
            {
                for (int y = 0; y < Height; y++)
                {
                    if (GetPixelsSegmentSize(x, y) > 0.01 * Width * Height)
                    image.SetPixel(x, y,
                        Color.FromArgb((int)(_segmentSaliencies[_pixelAssignments[x][y]] * 255),
                            (int)(_segmentSaliencies[_pixelAssignments[x][y]] * 255),
                            (int)(_segmentSaliencies[_pixelAssignments[x][y]] * 255)));
