...
namespace CP.FFTLibrary
{
    /// <summary>
    /// Defining Structure for Complex Data type  N=R+Ii
    /// https://www.codeproject.com/kb/gdi/fft.aspx
    /// </summary>
    struct COMPLEX
    {
    ...
    class FFT : IDisposable
    {
        public DirectBitmap Obj;               // Input Object Image
        public DirectBitmap FourierPlot;       // Generated Fouruer Magnitude Plot
        public DirectBitmap PhasePlot;         // Generated Fourier Phase Plot
        ...
        public float[,] FFTLog;                 // Log of Fourier Magnitude
        ...
        private void ReadImage()
        {
            ...
            DirectBitmap image = Obj;

            for (int x = 0; x < Width; x++)
            {
                for (int y = 0; y < Height; y++)
                {
                    GreyImage[x, y] = (int)(MyColorSpaceHelper.GetIntensityFromRGB(image.GetPixel(x, y)) * 255.0);
                }
            }
        }
        public DirectBitmap Displayimage()
        {
            ...
            for (int x = 0; x < Width; x++)
            {
                for (int y = 0; y < Height; y++)
                {
                    image.SetPixel(x, y, Color.FromArgb(GreyImage[x, y], GreyImage[x, y], GreyImage[x, y]));
                }
            }

            return image;// col;
        }
        public DirectBitmap Displayimage(int[,] image)
        {
            DirectBitmap output = new DirectBitmap(image.GetLength(0), image.GetLength(1));

            for (int x = 0; x < output.Width; x++)
            {
                for (int y = 0; y < output.Height; y++)
                {
                    int val = (int)(image[x, y] / 2000.0 * 255.0);
        ...
        */
        ...
        public void Dispose()
        {
            // Don't dispose Obj, the image passed in constructor
            if (FourierPlot != null) FourierPlot.Dispose();
            if (PhasePlot != null) PhasePlot.Dispose();
        }
    }
}
